...
const int defusingLedPin = 4;
const int armedLedPin = 5;
...
void setup()
{
    ...
    while (keycount < 4)
    {
        ...
        lcd.blink();
        char armcode = customKeypad.getKey();
        //armcode == NO_KEY;
    ...
    while (keycount == 12) {
        char armkey = customKeypad.getKey();
...
{
    timer();
    char disarmcode = customKeypad.getKey();
    ...
    if (disarmcode == '*') {
        ...
        if (keycount == 4) {
            if (entered[0] == password[0] && entered[1] == password[1] && entered[2] == password[2] && entered[3] == password[3]) {
                ...
                digitalWrite(defusingLedPin, HIGH);
                digitalWrite(armedLedPin, LOW);
...
//Serial.println(armcode);
 ...
 //   }
  ...
  if (buttonState == HIGH && isArmed == 1 && isDefused == 0)
  {
    button1Hold++;
    digitalWrite(defusingLedPin, HIGH);
  ...
  {
    button1Hold = 0;
    digitalWrite(defusingLedPin, LOW);
  }

  if (isArmed == 1 && isDefused == 0)
  {
    buzz();
  }*/
}